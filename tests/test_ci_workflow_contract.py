...
class CiWorkflowContractTests(unittest.TestCase):
    ...
    def test_pages_workflow_publishes_and_cleans_up_pr_previews(self) -> None:
        ...
        self.assertIn('PRODUCTION_URL: "https://hugo.opengis.ch/"', workflow)
        self.assertIn('PREVIEW_ROOT: "https://hugo.opengis.ch/pr-preview"', workflow)
        ...
        self.assertIn('find "${deploy_dir}/pr-preview" -mindepth 1 -maxdepth 1 -type d -name "pr-*" -print0', workflow)
        self.assertIn('"https://api.github.com/repos/${GITHUB_REPOSITORY}/pulls/${pr_number}"', workflow)
        self.assertIn('pr_response="$(curl -fsSL', workflow)
        self.assertIn('if [[ -z "${pr_response}" ]]; then', workflow)
        self.assertIn('pr_state="$(jq -r \'.state\' <<< "${pr_response}")"', workflow)
        self.assertIn('if [[ "${pr_state}" == "closed" ]]; then', workflow)
        self.assertIn('rm -rf "${preview_dir}"', workflow)
        self.assertTrue((REPO_ROOT / "static/CNAME").exists())
        self.assertEqual("hugo.opengis.ch", (REPO_ROOT / "static/CNAME").read_text(encoding="utf-8").strip())
        self.assertTrue((REPO_ROOT / "CNAME").exists())
        self.assertEqual("hugo.opengis.ch", (REPO_ROOT / "CNAME").read_text(encoding="utf-8").strip())
