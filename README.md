...
- Menu links should point to real page routes instead of Hugo alias routes, because alias pages redirect with the configured `baseURL` and can send preview visitors back to production. The courses calendar content lives under `content/pages/services/courses/calendar/`, but its public routes are canonicalized to `/courses-calendar/`, `/de/kurskalender/`, `/fr/calendrier-des-cours/`, and `/it/calendario-corsi/`.
...
Every `content/pages/index*.md` file keeps its normalized `source:` route covered either by the canonical `url:` or by `aliases`, and a regression test checks that content-page front matter stays valid and those source paths remain covered.
...
The workflow contract tests intentionally verify required CI behavior and runtime settings instead of exact GitHub Action major versions, so Dependabot can keep actions current without needing a test update for each action bump.
...
The `.github/workflows/pages.yml` deployment workflow publishes production builds from `main` to the `gh-pages` branch and publishes pull-request previews under `https://hugo.opengis.ch/pr-preview/pr-<number>/`, preserving the preview tree during production deploys while pruning preview folders for closed or merged pull requests. The dedicated pull-request close job also removes the matching preview immediately when GitHub emits the close event. Production deploys explicitly check out `main`, and all Pages Hugo build commands pass `--source "${GITHUB_WORKSPACE}"` so scheduled runs build the checked-out site config instead of any generated Pages output. It uses the same private theme submodule secret plus the repository `GITHUB_TOKEN` for pushing and PR comments, and the `CNAME` plus `static/CNAME` files keep the GitHub Pages custom domain set to `hugo.opengis.ch`. This deployment workflow intentionally does not run `npm ci`; CSS is already checked in.
