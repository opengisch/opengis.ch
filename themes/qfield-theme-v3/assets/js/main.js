...
function initializeApp() {
  ...
  if (navbar) {
    ...
    const shouldUseCompactNavbarAtRest = () => {
      if (navbar.dataset.homeHeroNav === 'true') {
        return false
      }
    ...
    const scrollThreshold = Number.parseInt(
      navbar.dataset.scrollThreshold || (navbar.dataset.homeHeroNav === 'true' ? '32' : '10'),
      10
    )
    ...
    const handleScroll = () => {
      ...
      const shouldBeScrolled = currentScrollY > scrollThreshold || shouldUseCompactNavbarAtRest()
  ...
  // Keep nested menu parents (e.g. "Courses") clickable on desktop.
  const submenuParentToggles = document.querySelectorAll('.navbar-default .dropdown-submenu > .dropdown-toggle[href]')
  if (submenuParentToggles.length > 0) {
  ...
  const revealItems = document.querySelectorAll('[data-reveal="fade-left"]')
